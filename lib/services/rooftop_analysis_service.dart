...
/// Can be backed by computer vision models, satellite LiDAR, or mock simulation.
...
class MockRooftopAnalysisService implements RooftopAnalysisService {
  ...
  MockRooftopAnalysisService({
    double defaultGrossArea = 1440.0,
    double defaultUsableArea = 1120.0,
  }) {
    _currentState = _createState(
      phase: AnalysisPhase.input,
      grossArea: defaultGrossArea,
      usableArea: defaultUsableArea,
      completed: [],
    );
  }
  ...
  AnalysisProgressState _createState({
    ...
    final capacity = SolarCalculatorService.recommendCapacityFromArea(
      RooftopAnalysis(
        ...
        obstacleAreaSqFt: grossArea - usableArea,
  ...
  Future<void> startAnalysis({
    ...
    final gross = initialGrossArea ?? 1440.0;
    final usable = initialUsableArea ?? 1120.0;
